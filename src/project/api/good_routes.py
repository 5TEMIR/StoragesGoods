...
from project.api.depends import database, good_repo
...
async def add_good(good_dto: GoodCreateUpdateSchema) -> GoodSchema:
    try:
        async with database.session() as session:
...
async def update_good(good_id: int, good_dto: GoodCreateUpdateSchema) -> GoodSchema:
    try:
        async with database.session() as session:
